...
export const Route = createFileRoute("/_authenticated/dashboard/images")({
...
function ImagesIndex() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        setItems(list);
        if (list.length === 1) navigate({ to: "/dashboard/images/$businessId", params: { businessId: list[0].id }, replace: true });
      });
  }, []);
  ...
  return (
    <div className="p-6 lg:p-10 max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-semibold tracking-tight">AI Images</h1>
        <p className="text-muted-foreground mt-1">Pick a business to generate brand-aware visuals.</p>
      </div>
      {items.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {items.map((b) => (
            <Link key={b.id} to="/dashboard/images/$businessId" params={{ businessId: b.id }} className="rounded-2xl border bg-card p-5 hover:shadow-elevated transition-shadow">
              ...
              <div className="mt-3 text-sm text-muted-foreground">Open generator →</div>
            </Link>
          ))}
        </div>
      )}
